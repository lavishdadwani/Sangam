...
import itemAPIs from "../../services/item"
...
const OwnerItemCard = ({data}) => {
    ...
    const handleDeleteItem = async () => {
        try {
          const result = await itemAPIs.deleteItem(data._id)
      
          if (result.ok) {
            dispatch(setOwnerData(result.data.data))
            dispatch(openSnackbar("Deleted Successfully", "success"));
          } else {
            dispatch(openSnackbar(result.data?.message || "Failed to delete item", "error"));
          }
      ...
      <div className='flex flex-col justify-between p-3 flex-1'>
        ...
        <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center gap-2'>
                ...
                <div className='p-2 cursor-pointer rounded-full hover:bg-[#ff4d2d]/10 text-[#ff4d2d] ' onClick={handleDeleteItem}>
                <FaTrashAlt size={16} />
                </div>
            </div>
        </div>
